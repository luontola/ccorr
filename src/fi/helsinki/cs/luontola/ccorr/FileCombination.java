...
/**
 * A class for combining many files in to one. After creating a new instance of 
 * this class, the <code>addItem</code> method should be used to determine which 
 * bytes should be read from which files. The files are read in the order that 
 * the items are added. The file this class represents can then be written or 
...
public class FileCombination {
    ...
    public void addItem(File file, long start, long end) {
        if (file != null && start >= 0 && end > start) {
            ...
            this.file.add(file);
            this.startOffset.add(new Long(start));
            this.endOffset.add(new Long(end));
        } else {
    ...
    public File getFile(int item) {
        if (item < 0 || item >= this.file.size()) {
            ...
            return (File) (this.file.elementAt(item));
        }
    }
    ...
    public long getStart(int item) {
        if (item < 0 || item >= this.startOffset.size()) {
            ...
            return ((Long) (this.startOffset.elementAt(item))).longValue();
        }
    }
    ...
    public long getEnd(int item) {
        if (item < 0 || item >= this.endOffset.size()) {
            ...
            return ((Long) (this.endOffset.elementAt(item))).longValue();
        }
    }
    ...
    /**
     * Writes the output of this <code>FileCombination</code> to a file.
    ...
    /**
     * Counts the checksum of this <code>FileCombination</code>'s output.
     * The operation can take some minutes depending on the size of the file 
     * to be processed. A ProgressMonitor is used if available (NOT IMPLEMENTED).
     *
    ...
    /**
     * Counts the checksums of an array of <code>FileCombination</code>s.
     * The operation can take some minutes depending on the size of the file 
     * to be processed. A ProgressMonitor is used if available (NOT IMPLEMENTED).
     *
        ...
        for (int item = 0; item < fc[0].getItems(); item++) {
            ...
            for (int i = 0; i < fc.length; i++) {
                if (!readFiles.contains(fc[i].getFile(item))) {     // if the file hasn't been read before
                    ...
                    CRC[] output = (CRC[]) (selectedCrcs.toArray(new CRC[0]));
    ...
    /**
     * Updates all the <code>CRC</code>s in an array 
     * with the data from an <code>InputStream</code>.
     *
     * @param   start   input's start offset
     * @param   end     input's end offset
     * @param   input   the input from which to read
